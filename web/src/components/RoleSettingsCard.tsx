...
import { getRelationRoles } from '../services/api'
...
export function RoleSettingsCard({
  ...
  useEffect(() => {
    if (userId) {
      void getRelationRolesList()
    }
  }, [userId])

  const getRelationRolesList = async () => {
    try {
      const res: any = await getRelationRoles({ id: userId })
      const {code, data} = res
      if (code === 200) {
        setSelectedRowKeys(data?.data.map((role: RolesRecord) => role.id) || [])
      } else {
        setSelectedRowKeys([])
      }
    } catch (error) {
      console.error('Failed to fetch relation roles:', error)
      setSelectedRowKeys([])
    }
  }
